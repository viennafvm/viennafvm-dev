  ...
  class linear_assembler
  {
    public:
                      ...
                      VectorT          & load_vector)
      {
        ...
        std::size_t map_index = viennafvm::create_mapping(pde_system, pde_index, segment);
    ...
    private:
                    ...
                    VectorT             & load_vector)
      {
        ...
        expr_type  partial_omega_integrand = extract_surface_integrand<FacetType>(integral_form.lhs(), u);
        expr_type   matrix_omega_integrand = extract_volume_integrand<CellType>(integral_form.lhs(), u);
        expr_type      rhs_omega_integrand = extract_volume_integrand<CellType>(integral_form.rhs(), u);
        expr_type  stabilization_integrand = prepare_for_evaluation<CellType>(pde_options.damping_term(), u);
        ...
        viennafvm::flux_handler<CellType, FacetType, interface_type>  flux(partial_omega_integrand, u);
        ...
        {
          long row_index = viennadata::access<viennafvm::mapping_key, long>(map_key)(*cit);
                                 ...
                                 ++focit)
          {
            ...
            if (other_cell)
            {
              long col_index = viennadata::access<viennafvm::mapping_key, long>(map_key)(*other_cell);
              ...
              {
                double boundary_value = viennadata::access<viennafvm::boundary_key, double>(bnd_key)(*other_cell);
                ...
                load_vector(row_index)              -= flux.out(*cit, *focit, *other_cell) * effective_facet_area * (boundary_value - current_value);
                system_matrix(row_index, row_index) -= flux.in(*cit, *focit, *other_cell) * effective_facet_area;

                load_vector(row_index) -= flux.out(*cit, *focit, *other_cell) * effective_facet_area * current_value;
                load_vector(row_index) += flux.in(*cit, *focit, *other_cell) * effective_facet_area * get_current_iterate(*cit, u);
              }
              else if (col_index >= 0)
              {
                ...
                load_vector(row_index) -= flux.out(*cit, *focit, *other_cell) * effective_facet_area * get_current_iterate(*other_cell, u);
                load_vector(row_index) += flux.in(*cit, *focit, *other_cell) * effective_facet_area * get_current_iterate(*cit, u);
              }
          ...
          load_vector(row_index) -= viennamath::eval(substituted_matrix_omega_integrand, p) * cell_volume * get_current_iterate(*cit, u);
